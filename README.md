...
pipeline {
    ...
    stages {
        ...
        stage ("Git checkout") {
            steps {
                git branch: 'main', url: 'https://github.com/Yashank-Sahu/DevSecOps-In-Action.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker') {
                        sh "docker tag background-remover-python-app yashank0611/devsecops-in-action:latest"
                        sh "docker push yashank0611/devsecops-in-action:latest"
                    }
                }
            }
        }
            ...
            steps {
                script {
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                       sh 'docker-scout quickview yashank0611/devsecops-in-action:latest'
                       sh 'docker-scout cves yashank0611/devsecops-in-action:latest'
                       sh 'docker-scout recommendations yashank0611/devsecops-in-action:latest'
                   }
                }
            }
        }
        stage ("Deploy to Container") {
            steps {
                sh 'docker run -d --name background-remover-python-app -p 5100:5100 yashank0611/devsecops-in-action:latest'
            }
        }
    }
    post {
